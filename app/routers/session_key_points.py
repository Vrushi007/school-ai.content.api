...
def create_session_key_points_bulk(bulk_data: SessionKeyPointBulkCreate, db: Session = Depends(get_db)):
...
def get_key_points_by_session(session_id: int, db: Session = Depends(get_db)):
